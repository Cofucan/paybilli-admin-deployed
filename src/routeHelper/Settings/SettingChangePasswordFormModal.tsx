...
const SettingChangePasswordFormModal: FC<SettingModalFormProps<SettingChangePasswordForm>> = ({ form, isModalVisible, setIsModalVisible, onSubmit }) => {
    ...
    return (
        <Modal isVisible={isModalVisible} toggleVisibility={toggleModalVisible} title={<h2 className="text-2xl font-bold">Password Settings</h2>} showClose={false}>
            <p className="text-gray-500">Update password for enhanced account Security</p>
            <form onSubmit={onPasswordChange} onReset={onPasswordChangeReset} className="space-y-5 my-5 min-w-96">
                <FormField>
                    <span>Current Password</span>
                    <FormField.Input type="password" intent={"admin"} placeholder="*******"    {...form.register(
                        "currentPassword",
                ...
                <div className="flex justify-between items-center">
                    ...
                    <FormField.Button intent={'admin'} themeSize={'3'} themeColor={"full-blue"}>Update Password</FormField.Button>
                </div>
            </form>
        </Modal>)
}
