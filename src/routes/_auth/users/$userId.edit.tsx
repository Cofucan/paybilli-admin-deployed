...
import { useUserChangePicture, useUserEdit, useUserGetById } from "../../../hooks/useUsersQuery";
...
function RouteComponent() {
  ...
  const editPictureMutate = useUserChangePicture();

  async function handleImageUpload(event: ChangeEvent<HTMLInputElement>) {
    const file = event.target.files?.[0];
    if (!file) return;
    await editPictureMutate.mutateAsync({ id: userId, file });
  }
      ...
      <form className='mt-6 grid grid-cols-2 gap-6' onSubmit={onSubmit} onReset={onReset}>
        <FormField intent={"admin"}>
          ...
          <FormField.Select
            ...
            options={(constantsQuery.data?.result.account_status ?? []).map((x) => ({
              title: x.display,
              value: x.value,
            }))}
            {...register("account_status", formErrorHelper("Status", { isRequired: true }))}
          />
        </FormField>
        <FormField intent={"admin"}>
