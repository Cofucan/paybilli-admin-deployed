    ...
    return (
        <div className='space-y-4 my-4'>
            ...
            <ul className='flex gap-6 '>
                {props.filterOptions.map((option, index) => <li onClick={() => { setHeaderIndex(index); option.onClick() }} key={index} className={`px-5 py-2 rounded cursor-pointer ${index === headerIndex ? "bg-slate-800": "border-slate-800"}`}>{option.header}</li>)}
            </ul>
            <button className='bg-slate-700 rounded border px-5 py-2 '>Bulk Actions</button>
        </div>
    )
}
