...
import { columns } from "../AdminTable/AdminColumn";
import { AdminData } from "./AdminData";


const AdminTable = () => {
  const [filterTickets, setfilterTickets] = useState(AdminData);
  ...
  const filterWithdrawals = (filter) => {
    ...
    setfilterTickets(
      actualFilter === "All"
        ? AdminData
        : AdminData.filter((admin) => admin.role === actualFilter)
    );
    ...
    <div className="p-6 bg-white overflow-x-auto">
      <div className="flex flex-col lg:flex-row justify-between items-center text-sm text-gray-500 mb-5 border border-[#58bff2] lg:border-gray-200 rounded-xl py-2 px-4">
        {[
          ...
          <button
            key={role}
            className={`mx-2 border-none px-5 py-3 ${getButtonClass(role)}`}
            onClick={() => filterWithdrawals(role)}
          >
            {role}
          </button>
        ))}
