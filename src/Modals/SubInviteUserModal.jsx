        ...
        <div className="w-full">
          ...
          <ul className="space-y-3">
            {roles.map((role, index) => (
              <li
                key={index}
                className={`border rounded-lg flex items-start px-3 py-2 justify-between ${
                  selectedRole === role.name ? "bg-[#eaecf0]" : ""
                } ${
                  role.name === "admin"
                    ? "disabled"
                    : "cursor-pointer"
                }`} // Disable cursor and add opacity for "admin"
                onClick={() => role.name !== "admin" && handleSelect(role.name)} // Prevent selecting "admin"
              >
...
export default SubInviteUsermModal;
